...
all_city = pd.read_csv('us-state-capitals.csv')  
    

#cleaning the text and forming dictionary which have city to which the state belongs to
all_city['description'] = all_city['description'].apply(cleanTxt)
all_city['state']= all_city['state'].apply(cleanTxt)
all_city_final = all_city[['description','state']]
STATES = all_city['state']
...
state_capital.drop(['state', 'latitude', 'longitude'],axis=1,inplace = True)
STATES1= pd.read_csv("us-state-capitals.csv")
STATES1.drop(['latitude', 'longitude','description'],axis=1,inplace = True)
...
app.title = 'Easim Twitter Sentiment Monitor'
...
app.layout = html.Div([
                     html.H1('Easim Twitter Sentiment Monitor', style={
        'textAlign': 'center'}),
        html.Img(src="https://raw.githubusercontent.com/cldougl/plot_images/add_r_img/vox.png"),    
...
def update_graph_bottom_live(n):
    ...
    fig1 = go.Figure(data=go.Choropleth(
        geojson="gz_2010_us_outline_500k.json",
        featureidkey='properties.ST_NM',
        locationmode='geojson-id',
        locations=final_state['description'],
                   ...
                   html.Div([
                        dcc.Graph(
                            ...
                            figure={
                                ...
                                'layout':{
                                    'showlegend':False,
                                    'title':'Currently tracking {} on Twitter'.format(search_words),
                                    'annotations':[
                                        dict(
